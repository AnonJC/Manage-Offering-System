...
$offeringno = pg_escape_string($_POST['offeringno']);
...
{
  $query = "UPDATE sis.offering SET lecturestart = time '$lecturestart', lectureend = time '$lectureend', lecturedays = '$lecturedays',
              section = '$section', slots = $slots, facultyid = '$faculty', roomid = $room, syearid = $syearid WHERE offeringno = $offeringno";
  ...
  if (!$result)
  {
      echo '<div class="alert alert-danger alert-dismissable" id="flash-msg" style="margin-top: 10%; margin-left: 15%; margin-right: 15%;">
        ...
        <br>
        <a href="dataupload.php" class="btn btn-danger" alt="" style="margin-left: 47%;"data-toggle="modal" data-target="#updatemodal">Back</a>
        </div>';
  }else{
 ...
 pg_close();
  ...
  $query = "UPDATE sis.offering SET lecturestart = time '$lecturestart', lectureend = time '$lectureend', labstart = time '$labstart',
            labend = time '$labend', lecturedays = '$lecturedays', labdays = '$labdays', section = '$section', slots = $slots, facultyid = '$faculty', roomid = $room, syearid = $syearid WHERE offeringno = $offeringno";
    $result = pg_query($query);
    if (!$result)
    {
        echo '<div class="alert alert-danger alert-dismissable" id="flash-msg" style="margin-top: 10%; margin-left: 15%; margin-right: 15%;">
